...
{#if showNotification}
    <div class="fixed top-4 right-4 z-50 animate-fade-in">
        <div
            class={`px-4 py-3 rounded shadow-lg ${notificationType === "success" ? "bg-green-500" : "bg-red-500"} text-white`}
        >
            {notificationMessage}
        </div>
    </div>
...
{#if showDeleteModal}
    <!-- Overlay: div with role="button" for accessibility -->
    <div
        role="button"
        tabindex="0"
...
{:else}
    <div class="space-y-3">
        {#each data.questions as q}
            <div
                class="bg-white border border-gray-200 rounded-md p-4 hover:shadow-sm transition"
            >
                <div class="flex items-center justify-between">
                    ...
                    <span
                        class="text-xs text-gray-600 border border-gray-300 rounded px-2 py-0.5"
                    >
                        {q.difficulty}
                    </span>
                </div>
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                    {#each q.tags as t}
                        <span
                            class="text-xs px-2 py-0.5 rounded bg-gray-100 text-gray-700 border border-gray-300"
                        >
                            {t}
                        </span>
                    {/each}
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    ...
                    {#if q.role === "owner"}
                        ...
                        <!-- Owner-only collaborator button -->
                        <a
                            href={`/dashboard/teacher/questions/${q.id}/collaborators`}
                            class="px-3 py-1.5 text-sm rounded bg-secondary text-white"
                        >
                            Collaborators
                        </a>
                    {/if}
