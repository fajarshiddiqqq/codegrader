...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    <div class="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div class="text-lg font-semibold text-gray-900 capitalize">{q.difficulty}</div>
    </div>
    <div class="bg-white border border-gray-200 rounded-lg p-4">
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div class="text-lg font-semibold text-gray-900 capitalize">{q.role}</div>
    </div>
</div>
...
{#if q.tags && q.tags.length > 0}
    <div class="bg-white border border-gray-200 rounded-lg p-4 mb-6">
        ...
        <div class="flex flex-wrap gap-2">
            {#each q.tags as tag}
                <span class="text-sm px-3 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                    {tag}
                </span>
            {/each}
        </div>
...
{#if q.starter_code}
    <div class="bg-white border border-gray-200 rounded-lg p-6 mb-6">
        ...
        <pre class="bg-gray-50 border border-gray-200 rounded p-4 overflow-x-auto text-sm"><code>{q.starter_code}</code></pre>
    </div>
{/if}
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6 mb-6">
        ...
        <div class="space-y-4">
            {#each q.test_cases as testCase, index}
                <div class="border border-gray-200 rounded-lg p-4 {testCase.is_hidden ? 'bg-amber-50 border-amber-200' : 'bg-gray-50'}">
                    <div class="flex items-center justify-between mb-3">
                        <h3 class="font-medium text-gray-900">Test Case #{index + 1}</h3>
                        {#if testCase.is_hidden}
                            <span class="text-xs px-2 py-1 rounded bg-amber-200 text-amber-800 font-medium">
                                Hidden
                            </span>
                        {:else}
                            <span class="text-xs px-2 py-1 rounded bg-green-100 text-green-800 font-medium">
                                Visible
                            </span>
                        {/if}
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <div class="text-xs text-gray-600 mb-1 font-medium">Input</div>
                            <pre class="bg-white border border-gray-200 rounded p-2 text-sm overflow-x-auto"><code>{testCase.input_data}</code></pre>
                        </div>
                        <div>
                            <div class="text-xs text-gray-600 mb-1 font-medium">Expected Output</div>
                            <pre class="bg-white border border-gray-200 rounded p-2 text-sm overflow-x-auto"><code>{testCase.expected_output}</code></pre>
                        </div>
                    </div>
                </div>
            {/each}
...
{#if q.rubric}
    <div class="bg-white border border-gray-200 rounded-lg p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-sm px-3 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200 capitalize">
                {q.rubric.tone} Tone
            </span>
        </div>
                ...
                {#each q.rubric.criteria as criterion}
                    <div class="border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center justify-between mb-2">
                            <h3 class="font-medium text-gray-900 capitalize">{criterion.aspect}</h3>
